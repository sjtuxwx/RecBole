...
class FairTrainDataLoader(NegSampleDataLoader):
...
class TrainDataLoader(NegSampleDataLoader):
    ...
    def collate_fn(self, index):
        ...
        data = self._dataset[index]
        transformed_data = self.transform(self._dataset, data)
        dd = self._neg_sampling(transformed_data)
        # dd.user2item = self._dataset.history_item_matrix()[0][data.user_id]
        # dd.itempop = self._dataset.history_user_matrix()[1][data.item_id].sum(axis=-1)
        return dd
